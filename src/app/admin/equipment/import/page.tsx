...
import { bulkImportEquipment, type EquipmentInput } from "@/lib/library-queries";
...
async function importAction(formData: FormData): Promise<{ created: number; updated: number; errors: { name: string; error: string }[] } | { parseError: string }> {
  ...
  let rows: EquipmentInput[];
  try {
    const parsed = JSON.parse(text);
    if (!Array.isArray(parsed)) throw new Error("Expected a JSON array of equipment objects.");
    rows = parsed;
  } catch (err) {
    return { parseError: err instanceof Error ? err.message : "Invalid JSON." };
  }
  return bulkImportEquipment(toLibrary ? null : campaignId, rows);
}

export default async function AdminEquipmentImportPage({
  searchParams,
}: {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    if ("parseError" in result) {
      redirect(`/admin/equipment/import?errors=${encodeURIComponent(JSON.stringify([{ name: "(file)", error: result.parseError }]))}`);
    }
    redirect(`/admin/equipment/import?created=${result.created}&updated=${result.updated}&errors=${encodeURIComponent(JSON.stringify(result.errors))}`);
  }
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="font-display text-2xl text-gold mb-2">Bulk Import Equipment</h1>
      <p className="text-sm text-parchment/40 mb-6">
        Paste a JSON array of item objects, or upload a .json file. Each object needs at least a{" "}
        <code className="text-gold/70">name</code>; <code className="text-gold/70">category</code>,{" "}
        <code className="text-gold/70">rarity</code>, <code className="text-gold/70">cost</code>,{" "}
        <code className="text-gold/70">weight</code>, <code className="text-gold/70">source</code> and{" "}
        <code className="text-gold/70">details</code> (statLine/description) are optional. Matching by name/slug
        means re-running an import updates existing items instead of duplicating them.
      </p>
      ...
      <form action={handleSubmit} className="space-y-4 rounded-lg border border-gold/15 p-4">
        ...
        <label className="block">
          ...
          <textarea name="json" rows={10} placeholder='[{"name": "Longsword", "category": "Weapon", "cost": "15 gp", "details": {"statLine": "Martial Melee - 1d8 slashing"}}]' className="w-full rounded-lg bg-void border border-gold/30 px-3 py-2 text-parchment text-xs font-mono focus:outline-none focus:border-gold/70" />
        </label>
